...
class CLNetworkManager: NSObject {
    func initateWebRequest(_ netWorkModel : CLNetworkModel, success:@escaping (_ result: Data,_ response:HTTPURLResponse)->(), failiure:@escaping (_ error:ErrorType)->()){
                    ...
                    if let error_ = error{
                        ...
                        DispatchQueue.main.async(execute: { () -> () in
                            ...
                            if let _data = data, let _response = response as? HTTPURLResponse{
                                if let status = _response.allHeaderFields[Constant.Names.ExpirationStatusKey] as? String{
